...
my_data = read.csv("C:/Users/carol/Downloads/perfomance_centrality_dataset_final - perfomance_centrality_dataset2.csv",head=TRUE)
...
lm1 = lm(Student.Achievement ~., data = my_data)
...
lm2 = lm(Student.Achievement ~., my_training_set)
...
plot(my_x, my_test_set$Student.Achievement, col = "blue", cex = 0.3)
lines(my_x, my_test_set$Student.Achievement, col = "blue")

points(my_x, my_test_set$pred, col = "red", cex=0.3, pch = 2)
lines(my_x, my_test_set$pred, col = "red")

my_test_set$error = my_test_set$Student.Achievement - my_test_set$pred 

my_test_set$abs_error = abs(my_test_set$Student.Achievement - my_test_set$pred)

my_test_set$abs_pct_error = ((my_test_set$abs_error)/(my_test_set$Student.Achievement))*100.0
